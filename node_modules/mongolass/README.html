...
</style><title>README</title></head><body><article class="markdown-body"><h2 id="mongolass"><a name="user-content-mongolass" href="#mongolass" class="headeranchor-link" aria-hidden="true"><span class="headeranchor"></span></a>Mongolass</h2>
<p><a href="https://npmjs.org/package/mongolass"><img alt="NPM version" src="https://img.shields.io/npm/v/mongolass.svg?style=flat-square" /></a><br />
<a href="https://travis-ci.org/mongolass/mongolass"><img alt="Build status" src="https://img.shields.io/travis/mongolass/mongolass.svg?style=flat-square" /></a><br />
<a href="https://david-dm.org/mongolass/mongolass"><img alt="Dependency Status" src="http://img.shields.io/david/mongolass/mongolass.svg?style=flat-square" /></a><br />
<a href="///D://workspace/aym-blog/node_modules/mongolass/LICENSE"><img alt="License" src="http://img.shields.io/npm/l/mongolass.svg?style=flat-square" /></a><br />
<a href="https://npmjs.org/package/mongolass"><img alt="Downloads" src="http://img.shields.io/npm/dm/mongolass.svg?style=flat-square" /></a></p>
...
<pre><code class="bash">$ npm i mongolass --save
</code></pre>
...
<p>ObjectId schema:</p>
<p><pre><code class="javascript">'use strict';
...
<strong>NB:</strong> You can pass <code>collName</code> as collection name.</p>
<h2 id="api"><a name="user-content-api" href="#api" class="headeranchor-link" aria-hidden="true"><span class="headeranchor"></span></a>API</h2>
<p>Same as <a href="http://mongodb.github.io/node-mongodb-native/2.2/api/Collection.html">node-mongodb-native</a>.</p>
<h2 id="mongolass-vs-mongoose"><a name="user-content-mongolass-vs-mongoose" href="#mongolass-vs-mongoose" class="headeranchor-link" aria-hidden="true"><span class="headeranchor"></span></a>Mongolass vs Mongoose</h2>
<p>知乎：<a href="https://zhuanlan.zhihu.com/p/24308524">从零开始写一个 Node.js 的 MongoDB 驱动库</a></p>
<hr />
<p>I&rsquo;ve been using Mongoose for years, it&rsquo;s great but complex sucks, so i wrote Mongolass. Mongolass is not simply mimicking Mongoose, but rather draw on the advantages of mongoose redesigned the architecture. Mongolass has some exciting features different from Mongoose:</p>
<ol>
<li>Pure Schema. In Mongoose, Schema and Model and Entity are confused.</li>
</ol>
<blockquote>
<p>Schemas not only define the structure of your document and casting of properties, they also define document instance methods, static Model methods, compound indexes and document lifecycle hooks called middleware.</p>
</blockquote>
<p>In Mongolass, Schema is only used for defining the structure of your document and casting of properties, Model used for retrievaling data from mongodb and register plugins, Entity(as result) is plain object. Schema is also optional.</p>
<ol>
<li>Awesome plugin system. Mongoose plugin system is not strong enough, eg: <code>.pre</code>, <code>.post</code>, use async <code>next()</code>. In Mongolass, we can register a plugin for Model or global mongolass instance. like:</li>
</ol>
<pre><code class="javascript">User.plugin('xx', {
...
<p>Above added two hook functions for <code>User</code>, when <code>User.find().xx().exec()</code> is called, the execution order is as follows:</p>
<pre><code>beforeFind(handle query args) -&gt; retrieve data from mongodb -&gt; afterFind(handle query result)
</code></pre>

<p>Mongolass&rsquo;s plugins could be substituted for Mongoose&rsquo;s (document instance methods + static Model methods + plugins).</p>
<ol>
<li>Detailed error informations. see <a href="https://github.com/mongolass/mongolass#usage">usage</a>.</li>
</ol>
...
<p>According to the error instance, esay to know <code>age</code> expect a number but got a string, from error stack know it&rsquo;s broken on <code>app.js:23:4</code> and the operator is <code>Model.insertOne</code>.</p>
<h2 id="schema"><a name="user-content-schema" href="#schema" class="headeranchor-link" aria-hidden="true"><span class="headeranchor"></span></a>Schema</h2>
...
<pre><code>mongolass.plugin(pluginName, hooks);// register global plugin
...
<p><strong>NOTE</strong>: Different order of calling plugins will output different results. The priority of Model&rsquo;s plugins is greater than global&rsquo;s.</p>
<p>example:</p>
<pre><code class="javascript">const co = require('co');
const Mongolass = require('mongolass');
const mongolass = new Mongolass('mongodb://localhost:27017/test');
...
<p>see <a href="https://github.com/mongolass/mongolass-plugin-populate">mongolass-plugin-populate</a>.</p>
<h3 id="built-in-plugins"><a name="user-content-built-in-plugins" href="#built-in-plugins" class="headeranchor-link" aria-hidden="true"><span class="headeranchor"></span></a>Built-in plugins</h3>
...
<h2 id="test"><a name="user-content-test" href="#test" class="headeranchor-link" aria-hidden="true"><span class="headeranchor"></span></a>Test</h2>
<pre><code class="bash">$ npm test (coverage 100%)
</code></pre>
